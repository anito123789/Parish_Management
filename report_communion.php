...
if ($family_id && !$id) {
    ...
    $stmt = $db->prepare("SELECT p.name, p.dob, p.relationship, p.father_name, p.mother_name, p.communion_date as p_communion_date, c.* 
                          FROM parishioners p 
                          LEFT JOIN first_communions c ON p.id = c.parishioner_id 
                          WHERE p.family_id = ? AND (p.communion_date IS NOT NULL AND p.communion_date != '' AND p.communion_date != '0000-00-00') 
                          ORDER BY p.dob ASC");
    $stmt->execute([$family_id]);
    ...
    <head>
        <meta charset="UTF-8">
        <title>Family First Communion Report - <?php echo htmlspecialchars($family['name']); ?></title>
        <style>
            @page {
                size: A4 portrait;
                margin: 15mm;
    ...
    <body>
        <div class="no-print" style="margin: 20px; text-align: right;">
            <button onclick="window.print()"
                style="padding: 8px 16px; background: #b45309; color: white; border: none; border-radius: 6px; cursor: pointer;">Print
                Report</button>
        </div>
        <div class="header">
            <h1><?php echo htmlspecialchars($profile['church_name'] ?? 'Parish'); ?></h1>
            <p><?php echo htmlspecialchars(($profile['place'] ?? '') . ' | ' . ($profile['diocese'] ?? '')); ?></p>
            <h2 style="color: #b45309; margin-top: 5px; font-size: 14pt;">Family First Holy Communion Records</h2>
        ...
        <table>
            <thead>
                <tr>
                    ...
                    <th style="width: 23%;">Parents</th>
                    <th style="width: 12%;">Communion Date</th>
                    <th style="width: 18%;">Place</th>
                    <th style="width: 15%;">Minister</th>
                </tr>
            </thead>
            <tbody>
                <?php foreach ($members as $m): ?>
                    <tr>
                        ...
                        <td><?php echo format_date($m['p_communion_date']); ?></td>
...
// Fetch Parish Profile (for header and certification text)
$profile = $db->query("SELECT * FROM parish_profile LIMIT 1")->fetch() ?: [];
...
<head>
    <meta charset="UTF-8">
    <title>First Communion Certificate - <?php echo htmlspecialchars($p['name']); ?></title>
    <style>
        @page {
            size: A4 portrait;
            margin: 0;
...
<body>
    <div class="no-print" style="text-align:center; padding: 20px;">
        <button onclick="window.print()" class="btn btn-primary"
            style="padding: 10px 30px; font-size: 1.2rem; cursor: pointer; background: #2563eb; color: white; border: none; border-radius: 8px;">🖨️
            Print Certificate</button>
    </div>

    <div class="edit-notice">
        <strong>💡 Live Preview:</strong> You can click on any underlined field to edit. Click on <strong>Seal</strong>
    ...
    <div class="container">
        <div class="border-wrapper">
            ...
            <div class="title">
                <h3>Certificate of First Holy Communion</h3>
            </div>
            ...
            <table class="content-table">
                ...
                <tr>
                    ...
                    <td class="value" contenteditable="true">
                        <?php echo htmlspecialchars($communion['place'] ?? $c_name); ?>
                    </td>
                </tr>
                <tr>
                    <td class="label">Date of First Communion</td>
                    <td class="colon">:</td>
            ...
            <div class="certification-text">
                This is a true copy of the entry kept in the First Communion Register at <br>
            ...
            <div class="footer">
                <div class="footer-item" style="text-align: left; width: 45%;">
                    <div style="margin-bottom: 40px;" contenteditable="true">Place:
