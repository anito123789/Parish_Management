...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['save_voucher'])) {
    ...
    $acc_head = $_POST['account_head'];
    $cat = $_POST['category'] ?: 'Expense';

    if ($v_id) {
        $stmt = $db->prepare("UPDATE vouchers SET voucher_no=?, voucher_date=?, person_name=?, particulars=?, amount=?, amount_words=?, payment_mode=?, account_head=?, category=? WHERE id=?");
        $stmt->execute([$v_no, $v_date, $person, $particulars, $amount, $words, $mode, $acc_head, $cat, $v_id]);
        $msg = "Voucher Updated Successfully";
    } else {
        $stmt = $db->prepare("INSERT INTO vouchers (voucher_type, voucher_no, voucher_date, person_name, particulars, amount, amount_words, payment_mode, account_head, category) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
        $stmt->execute([$v_type, $v_no, $v_date, $person, $particulars, $amount, $words, $mode, $acc_head, $cat]);
...
if (!$v_data) {
    try {
        ...
        $v_no_default = 'EV-' . date('Ymd') . '-' . sprintf('%03d', $max_id + 1);
    } catch (Exception $e) {
        $v_no_default = 'EV-' . date('Ymd') . '-001';
    }
} else {
...
<div class="no-print"
    ...
    <form method="POST" id="voucherForm" onsubmit="return syncVoucherData()">
        ...
        <input type="hidden" name="particulars" id="hidden_particulars">
        <input type="hidden" name="payment_mode" id="hidden_mode">
        <input type="hidden" name="account_head" id="hidden_acc">
        ...
        <button type="submit" class="btn btn-danger" style="padding: 10px 30px; font-size: 1.1rem; cursor: pointer;">
    ...
    <div class="voucher-box">
        ...
        <div class="voucher-body">
            ...
            <div class="voucher-row mt-10">
                <div class="v-cell w-full">
                    ...
                    <span id="ui_particulars" class="border-b"
                        contenteditable="true"><?php echo htmlspecialchars($v_data['particulars'] ?? ''); ?></span>
                </div>
            </div>
            ...
            <div class="voucher-row mt-10">
                <div class="v-cell one-half">
                    ...
                    <span id="ui_acc" class="border-b"
                        contenteditable="true"><?php echo htmlspecialchars($v_data['account_head'] ?? ''); ?></span>
                </div>
                <div class="v-cell one-half">
                    <span class="label-text" style="margin-left: 10px;">Mode:</span>
            ...
            <div class="voucher-row">
                <div class="v-cell w-full">
                    ...
                    <span id="ui_category" class="border-b"
                        contenteditable="true"><?php echo htmlspecialchars($v_data['category'] ?? 'Expense'); ?></span>
                </div>
            </div>
        </div>
        ...
        <div class="voucher-footer">
            <div class="footer-grid">
                ...
                <div class="sig-wrap">
                    <div class="sig-item">
                        <div class="sig-line" style="margin-bottom: 40px;">Receiver's Signature</div>
                    </div>
                    <div class="sig-item">
...
<script>
    ...
    function syncVoucherData() {
        ...
        document.getElementById('hidden_particulars').value = document.getElementById('ui_particulars').innerText;
        document.getElementById('hidden_mode').value = document.getElementById('ui_mode').innerText;
        document.getElementById('hidden_acc').value = document.getElementById('ui_acc').innerText;
